  ...
  {  
    {
      ...
      if (global.server.state == LEADER) {
        {
          scope( scopeConnectionA )
          {
            install( IOException => println@Console(OutputNodeA.location + " ha rifiutato la connessione. Server offline.")() );
            appendEntries @ OutputNodeA (global.appendEntriesReq) (appendEntriesRespA);
            if ( appendEntriesRespA.success == false) {
              global.server.currentTerm = appendEntriesRespA.term;
              global.server.state = FOLLOWER;
              global.server.votedFor = -1;
...
init {
  ...
  global.server.location = "socket://localhost:8006";
      ...
      {
        {
          acquire@SemaphoreUtils( semRequestVote)( res );//prendo il lock sulla richiesta di voto;
          {
            scope( scopeConnectionA )
            {
              ...
              install( IOException => println@Console(OutputNodeA.location + " ha rifiutato la connessione. Server offline.")() );
              requestVote@OutputNodeA(global.requestVoteReq)(requestVoteResA);

              if ( requestVoteResA.voteGranted ) {
                voteCount++
                //log@DebugLogService(("Got vote from " + requestVoteReq.servers[i]) { .server = NODE_LOCATION } )
              };

              if (is_defined( requestVoteResA.term )) {
                serversCount++
              };
